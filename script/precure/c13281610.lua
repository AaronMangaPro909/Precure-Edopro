...
-- !!! REPLACE THESE WITH THE ACTUAL 8-DIGIT CARD IDS FROM YOUR DATABASE !!!
...
function s.initial_effect(c)
    -- Activate: Special Summon up to 4 specific monsters + Draw if you control 4+ Precures
...
-- Archetype Filter (0xb54)
function s.precure_filter(c)
    return c:IsFaceup() and c:IsSetCard(0xb54)
end

-- =========================================================
-- COST: Discard 1, then Banish 1 from Hand or Field
-- =========================================================
function s.cost(e, tp, eg, ep, ev, re, r, rp, chk)
    local c = e:GetHandler()
    -- Check if we can discard 1 card
    if chk == 0 then return Duel.IsExistingMatchingCard(Card.IsDiscardable, tp, LOCATION_HAND, 0, 1, c) end
    
    -- 1. Discard 1 card
    Duel.DiscardHand(tp, Card.IsDiscardable, 1, 1, REASON_COST + REASON_DISCARD, c)
    
    -- 2. Banish 1 card from hand or field (excluding this spell if activated from hand)
...
-- =========================================================
-- TARGET & OPERATION: Special Summon & Draw 2
-- =========================================================
...
function s.activate(e, tp, eg, ep, ev, re, r, rp)
    -- Apply the Xenophobic Special Summon restriction for the rest of the turn
    local e1 = Effect.CreateEffect(e:GetHandler())
    e1:SetType(EFFECT_TYPE_FIELD)
    e1:SetProperty(EFFECT_FLAG_PLAYER_TARGET + EFFECT_FLAG_CLIENT_HINT)
    e1:SetDescription(aux.Stringid(id, 0)) -- "Cannot Special Summon monsters, except Precure monsters"
    ...
    if ft > 4 then ft = 4 end -- Maximum of 4 monsters allowed by card text
    ...
    if #g > 0 then
        ...
        -- Select up to 'ft' monsters with distinct codes (optional design choice, allows duplicates up to 'ft' if you want)
        local sg = g:Select(tp, 1, ft, nil)
        if #sg > 0 and Duel.SpecialSummon(sg, 0, tp, tp, false, false, POS_FACEUP) > 0 then
            
            -- Draw Condition Check: Control 4 or more "Precure" monsters
